 ...
 *   - 标签名（start_tag / self_closing_tag 的 tag_name 子节点）
 *
 * tree-sitter 的 HTML 语言描述符通过 tree_sitter_html() 获取。
 */
class HtmlExtractor : public LanguageExtractor {
public:
  HtmlExtractor();
  ~HtmlExtractor() override;

  ExtractionResult extract(const std::string &file_path,
                           const std::string &source) override;
  const char *language_name() const override { return "html"; }
 ...
 *   - 方法定义（def name / def self.name / singleton_method）
 *   - 类定义（class Name）
 *   - 模块定义（module Name）
 *   - 单例类定义（class << obj）
 ...
 */
class RubyExtractor : public LanguageExtractor {
public:
  RubyExtractor();
  ~RubyExtractor() override;

  ExtractionResult extract(const std::string &file_path,
                           const std::string &source) override;
  const char *language_name() const override { return "ruby"; }
 ...
 *   - 预处理包含指令（#include → preproc_include）
 *   - 函数调用关系
 *
 * tree-sitter 的 GLSL 语言描述符通过 tree_sitter_glsl() 获取。
 */
class GlslExtractor : public LanguageExtractor {
public:
  GlslExtractor();
  ~GlslExtractor() override;

  ExtractionResult extract(const std::string &file_path,
                           const std::string &source) override;
  const char *language_name() const override { return "glsl"; }
