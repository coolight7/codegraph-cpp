...
public:
   ...
   */
  void insert_nodes_batch(const std::vector<Node> &nodes,
   ...
   *
   * 使用场景：
   *   - 批量索引完成后，确保 FTS 索引与 nodes 表同步
   *   - 触发器的 FTS 更新可能在某些情况下不完整
   *
   * 实现：使用 INSERT INTO nodes_fts(nodes_fts) VALUES('rebuild')
   * 来触发 FTS5 的完整重建（仅对 content= 外部内容表有效）。
   */
  void rebuild_fts();
