 ...
 *   2. build_context()  — 构建符号的完整上下文（定义 + callers + callees +
 * methods）
...
Json ContextBuilder::node_to_json(const Node &node) {
  Json j = {{"kind", node_kind_str(node.kind)},
            {"name", node.name},
            {"file", node.file_path},
            {"line", node.line}};
 ...
 */
Json ContextBuilder::build_context(const std::string &symbol, int limit,
                                   int max_depth) {
  // 先精确匹配，再 FTS 模糊搜索
  ...
  if (target.kind == NodeKind::Class || target.kind == NodeKind::Struct) {
    ...
    // 如果类在头文件中，还要查找对应的源文件
    auto try_add_source = [&](const std::string &header_ext,
                              const std::string &source_ext) {
      if (target.file_path.size() >= header_ext.size() &&
          target.file_path.substr(target.file_path.size() -
                                  header_ext.size()) == header_ext) {
        std::string source_file =
            target.file_path.substr(0, target.file_path.size() -
                                           header_ext.size()) +
            source_ext;
...
Json ContextBuilder::get_status() {
  return {{"node_count", db_.count_nodes()},
