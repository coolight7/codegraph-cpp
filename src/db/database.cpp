 ...
 */
static sqlite3_stmt *prepare_or_throw(sqlite3 *db, const char *sql,
                                      const std::string &op) {
 ...
 */
static void expect_done(sqlite3 *db, sqlite3_stmt *stmt,
                        const std::string &op) {
 ...
 * 如果返回非预期状态码则抛异常。
 */
static int step_or_throw(sqlite3 *db, sqlite3_stmt *stmt,
                         const std::string &op) {
...
void Database::finalize_cached_stmts() {
  if (stmt_insert_node_) {
    sqlite3_finalize(stmt_insert_node_);
    stmt_insert_node_ = nullptr;
  }
  if (stmt_insert_edge_) {
    sqlite3_finalize(stmt_insert_edge_);
    stmt_insert_edge_ = nullptr;
  }
  if (stmt_insert_unresolved_) {
    sqlite3_finalize(stmt_insert_unresolved_);
    stmt_insert_unresolved_ = nullptr;
  }
}
...
Database::Database(Database &&other) noexcept
    : db_(other.db_), stmt_insert_node_(other.stmt_insert_node_),
...
Node Database::read_node_row(sqlite3_stmt *stmt) {
  ...
  n.file_path = reinterpret_cast<const char *>(
      sqlite3_column_text(stmt, 4) ? sqlite3_column_text(stmt, 4)
                                   : (const unsigned char *)"");
  n.language = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 5)
  ...
  n.signature = reinterpret_cast<const char *>(
      sqlite3_column_text(stmt, 10) ? sqlite3_column_text(stmt, 10)
                                    : (const unsigned char *)"");
...
std::optional<Node> Database::get_node(int64_t id) {
  const char *sql =
      ...
      "is_const, is_exported FROM nodes WHERE id=?";
...
std::vector<Node> Database::find_nodes_by_name(const std::string &name,
  ...
  auto append_rows = [&](const char *sql, auto bind_fn, int max_rows) {
    ...
    int rc = SQLITE_OK;
    while ((rc = step_or_throw(db_, stmt, "find_nodes_by_name")) ==
           SQLITE_ROW) {
...
std::vector<Node> Database::find_nodes_by_file(const std::string &file_path) {
  ...
  const char *sql =
      "SELECT id, kind, name, qualified_name, file_path, language, line, col, "
      "end_line, end_col, signature, docstring, visibility, is_static, "
      "is_const, is_exported FROM nodes WHERE file_path=?";
...
int64_t Database::insert_edge(const Edge &edge) {
  const char *sql = "INSERT INTO edges (source_id, target_id, kind, line, col, "
                    "metadata) VALUES (?, ?, ?, ?, ?, ?)";
...
std::vector<Edge> Database::get_edges_from(int64_t source_id, EdgeKind kind) {
  ...
  const char *sql = "SELECT id, source_id, target_id, kind, line, col, "
                    "metadata FROM edges WHERE source_id=? AND kind=?";
  ...
  while ((rc = step_or_throw(db_, stmt, "get_edges_from")) == SQLITE_ROW) {
    ...
    e.metadata = reinterpret_cast<const char *>(
...
std::vector<Edge> Database::get_edges_to(int64_t target_id, EdgeKind kind) {
  std::vector<Edge> results;
  const char *sql = "SELECT id, source_id, target_id, kind, line, col, "
                    "metadata FROM edges WHERE target_id=? AND kind=?";
...
std::vector<Edge> Database::get_all_edges_from(int64_t source_id) {
  std::vector<Edge> results;
  const char *sql = "SELECT id, source_id, target_id, kind, line, col, "
                    "metadata FROM edges WHERE source_id=?";
...
int64_t Database::count_edges() {
  sqlite3_stmt *stmt = prepare_or_throw(db_, "SELECT COUNT(*) FROM edges",
                                        "prepare count_edges");
  StmtGuard guard(stmt);
...
int64_t Database::insert_file(const FileRecord &file) {
  const char *sql = "INSERT OR REPLACE INTO files (path, language, mtime, "
                    "size) VALUES (?, ?, ?, ?)";
...
std::optional<FileRecord> Database::get_file(const std::string &path) {
  const char *sql =
      "SELECT id, path, language, mtime, size FROM files WHERE path=?";
  ...
  f.language = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 2)
                                                  ? sqlite3_column_text(stmt, 2)
                                                  : (const unsigned char *)"");
...
std::vector<FileRecord> Database::get_all_files() {
  ...
  while ((rc = step_or_throw(db_, stmt, "get_all_files")) == SQLITE_ROW) {
    ...
    f.id = sqlite3_column_int64(stmt, 0);
    f.path = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 1));
    f.language = reinterpret_cast<const char *>(
...
void Database::delete_edges_for_file_nodes(const std::string &file_path) {
  const char *sql =
      R"(DELETE FROM edges WHERE source_id IN (SELECT id FROM nodes WHERE file_path=?)
        OR target_id IN (SELECT id FROM nodes WHERE file_path=?))";
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare delete_edges_for_file_nodes");
  StmtGuard guard(stmt);
  sqlite3_bind_text(stmt, 1, file_path.c_str(), -1, SQLITE_TRANSIENT);
...
void Database::delete_nodes_by_file(const std::string &file_path) {
  ...
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare delete_nodes_by_file");
  StmtGuard guard(stmt);
  sqlite3_bind_text(stmt, 1, file_path.c_str(), -1, SQLITE_TRANSIENT);
...
void Database::delete_unresolved_refs_by_file(const std::string &file_path) {
  const char *sql =
      R"(DELETE FROM unresolved_refs WHERE source_node_id IN (SELECT id FROM nodes WHERE file_path=?))";
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare delete_unresolved_refs_by_file");
  StmtGuard guard(stmt);
  sqlite3_bind_text(stmt, 1, file_path.c_str(), -1, SQLITE_TRANSIENT);
...
int64_t Database::insert_unresolved_ref(const UnresolvedRef &ref) {
  const char *sql = "INSERT INTO unresolved_refs (source_node_id, ref_name, "
                    "ref_kind, line, col) VALUES (?, ?, ?, ?, ?)";
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare insert_unresolved_ref");
  StmtGuard guard(stmt);
...
std::vector<UnresolvedRef> Database::get_unresolved_refs() {
  ...
  const char *sql = "SELECT id, source_node_id, ref_name, ref_kind, line, col "
                    "FROM unresolved_refs";
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare get_unresolved_refs");
  StmtGuard guard(stmt);
  int rc = SQLITE_OK;
  while ((rc = step_or_throw(db_, stmt, "get_unresolved_refs")) == SQLITE_ROW) {
    ...
    r.ref_kind = reinterpret_cast<const char *>(
        sqlite3_column_text(stmt, 3) ? sqlite3_column_text(stmt, 3)
                                     : (const unsigned char *)"");
...
void Database::delete_unresolved_ref(int64_t ref_id) {
  ...
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare delete_unresolved_ref");
  StmtGuard guard(stmt);
...
std::vector<Node> Database::get_nodes_by_ids(const std::vector<int64_t> &ids) {
  ...
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql.c_str(), "prepare get_nodes_by_ids");
  StmtGuard guard(stmt);
  for (size_t i = 0; i < ids.size(); ++i) {
...
std::vector<Node> Database::search_fts(const std::string &query, int limit) {
  ...
  const char *sql =
      R"(SELECT n.id, n.kind, n.name, n.qualified_name, n.file_path, n.language, n.line, n.col, n.end_line, n.end_col, n.signature, n.docstring, n.visibility, n.is_static, n.is_const, n.is_exported
  ...
  // Kind-based re-ranking: Functions/Classes first, then Variables, last
  // Imports
...
std::vector<Node> Database::find_dead_code(const std::string &exclude_pattern) {
  ...
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql.c_str(), "prepare find_dead_code");
  StmtGuard guard(stmt);
...
int64_t Database::count_files() {
  sqlite3_stmt *stmt = prepare_or_throw(db_, "SELECT COUNT(*) FROM files",
                                        "prepare count_files");
  StmtGuard guard(stmt);
 ...
 */
void Database::insert_nodes_batch(const std::vector<Node> &nodes,
  ...
  if (!stmt_insert_node_) {
    const char *sql = R"(INSERT INTO nodes
            (kind, name, qualified_name, file_path, language, line, col, end_line, end_col, signature, docstring, visibility, is_static, is_const, is_exported)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?))";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt_insert_node_, nullptr) !=
        SQLITE_OK) {
...
void Database::insert_edges_batch(const std::vector<Edge> &edges) {
  if (edges.empty())
    return;
  if (!stmt_insert_edge_) {
    const char *sql = "INSERT INTO edges (source_id, target_id, kind, line, "
                      "col, metadata) VALUES (?, ?, ?, ?, ?, ?)";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt_insert_edge_, nullptr) !=
        SQLITE_OK) {
  ...
  for (const auto &edge : edges) {
    ...
    sqlite3_bind_text(stmt_insert_edge_, 6, edge.metadata.c_str(), -1,
                      SQLITE_TRANSIENT);
...
void Database::insert_unresolved_batch(const std::vector<UnresolvedRef> &refs) {
  ...
  if (!stmt_insert_unresolved_) {
    const char *sql = "INSERT INTO unresolved_refs (source_node_id, ref_name, "
                      "ref_kind, line, col) VALUES (?, ?, ?, ?, ?)";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt_insert_unresolved_, nullptr) !=
        SQLITE_OK) {
  ...
  for (const auto &ref : refs) {
    ...
    sqlite3_bind_text(stmt_insert_unresolved_, 2, ref.ref_name.c_str(), -1,
                      SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt_insert_unresolved_, 3, ref.ref_kind.c_str(), -1,
                      SQLITE_TRANSIENT);
 ...
 */
void Database::delete_unresolved_refs_batch(
    const std::vector<int64_t> &ref_ids) {
  if (ref_ids.empty())
    return;
  const char *sql = "DELETE FROM unresolved_refs WHERE id=?";
  sqlite3_stmt *stmt =
      prepare_or_throw(db_, sql, "prepare delete_unresolved_refs_batch");
  StmtGuard guard(stmt);
