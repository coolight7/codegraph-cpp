/**
 * csharp_extractor.cpp — C# 语言提取器实现
 *
 * 使用 tree-sitter 解析 C# 源代码，提取：
 *   - 类/结构体/接口定义（class / struct / interface）
 *   - 命名空间定义（namespace）
 *   - 方法定义
 *   - 构造函数
 *   - using 指令
...
CSharpExtractor::CSharpExtractor() : lang_(tree_sitter_c_sharp()) {}
CSharpExtractor::~CSharpExtractor() = default;

std::string CSharpExtractor::get_node_text(TSNode node,
...
static NodeKind classify_csharp_node(const char *type_name) {
  if (strcmp(type_name, "class_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "struct_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "interface_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "namespace_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "method_declaration") == 0)
    return NodeKind::Method;
  if (strcmp(type_name, "constructor_declaration") == 0)
    return NodeKind::Method;
  if (strcmp(type_name, "using_directive") == 0)
    return NodeKind::Import;
  return NodeKind::Variable;
}

void CSharpExtractor::walk_tree(TSNode node, const std::string &source,
                                const std::string &file_path, int64_t parent_id,
                                ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_csharp_node(type_name);
  ...
  if (is_interesting) {
    ...
    if (kind == NodeKind::Import && n.name.empty()) {
      n.name = get_node_text(node, source);
      const char *prefix = "using ";
      if (n.name.rfind(prefix, 0) == 0)
        n.name = n.name.substr(strlen(prefix));
      size_t semi = n.name.rfind(';');
      if (semi != std::string::npos)
        n.name = n.name.substr(0, semi);
      if (n.name.size() > 80)
  ...
  if (kind == NodeKind::Method && my_id != parent_id) {
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "invocation_expression") == 0) {
...
ExtractionResult CSharpExtractor::extract(const std::string &file_path,
                                          const std::string &source) {
  ExtractionResult result;
