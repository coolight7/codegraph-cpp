/**
 * java_extractor.cpp — Java 语言提取器实现
 *
...
JavaExtractor::JavaExtractor() : lang_(tree_sitter_java()) {}
JavaExtractor::~JavaExtractor() = default;

std::string JavaExtractor::get_node_text(TSNode node,
...
static NodeKind classify_java_node(const char *type_name) {
...
static std::string get_java_method_name(TSNode node,
                                        const std::string &source) {
  const char *node_type = ts_node_type(node);
  uint32_t count = ts_node_child_count(node);

  if (strcmp(node_type, "method_declaration") == 0 ||
      strcmp(node_type, "constructor_declaration") == 0) {
...
void JavaExtractor::walk_tree(TSNode node, const std::string &source,
                              const std::string &file_path, int64_t parent_id,
                              ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_java_node(type_name);
  ...
  if (is_interesting) {
    ...
    if (kind == NodeKind::Method) {
      n.name = get_java_method_name(node, source);
      if (n.name.empty()) {
  ...
  if (kind == NodeKind::Method && my_id != parent_id) {
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "method_invocation") == 0) {
        ...
        if (!ts_node_is_null(name_node)) {
          std::string callee = get_node_text(name_node, source);
          if (!callee.empty()) {
            UnresolvedRef ref;
...
ExtractionResult JavaExtractor::extract(const std::string &file_path,
                                        const std::string &source) {
  ExtractionResult result;
