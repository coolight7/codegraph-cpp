...
LuaExtractor::LuaExtractor() : lang_(tree_sitter_lua()) {}
LuaExtractor::~LuaExtractor() = default;

std::string LuaExtractor::get_node_text(TSNode node,
...
static NodeKind classify_lua_node(const char *type_name) {
...
void LuaExtractor::walk_tree(TSNode node, const std::string &source,
                             const std::string &file_path, int64_t parent_id,
                             ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_lua_node(type_name);
  ...
  if (kind == NodeKind::Function && my_id != parent_id) {
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "function_call") == 0) {
        TSNode fn = ts_node_child_by_field_name(n, "name", 4);
...
ExtractionResult LuaExtractor::extract(const std::string &file_path,
                                       const std::string &source) {
  ExtractionResult result;
