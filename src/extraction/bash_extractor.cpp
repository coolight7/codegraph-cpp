/**
 * bash_extractor.cpp — Bash 语言提取器实现
 *
 * 使用 tree-sitter 解析 Bash 脚本，提取：
 *   - 函数定义（function / name()）
 *   - 命令调用关系
 *
 * Bash 提取器比 C++ 提取器简单，没有类/作用域追踪。
 * 调用关系通过 command 节点的 "name" 字段提取。
 */
...
BashExtractor::BashExtractor() : lang_(tree_sitter_bash()) {}
BashExtractor::~BashExtractor() = default;

std::string BashExtractor::get_node_text(TSNode node,
...
static NodeKind classify_bash_node(const char *type_name) {
  if (strcmp(type_name, "function_definition") == 0)
    return NodeKind::Function;
  if (strcmp(type_name, "variable_assignment") == 0)
    return NodeKind::Variable;
  return NodeKind::Variable;
}

void BashExtractor::walk_tree(TSNode node, const std::string &source,
                              const std::string &file_path, int64_t parent_id,
                              ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_bash_node(type_name);
...
ExtractionResult BashExtractor::extract(const std::string &file_path,
                                        const std::string &source) {
  ExtractionResult result;
