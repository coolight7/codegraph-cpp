...
PhpExtractor::PhpExtractor() : lang_(tree_sitter_php()) {}
PhpExtractor::~PhpExtractor() = default;

std::string PhpExtractor::get_node_text(TSNode node,
...
static NodeKind classify_php_node(const char *type_name) {
  ...
  if (strcmp(type_name, "namespace_use_declaration") == 0)
    return NodeKind::Import;
  return NodeKind::Variable;
}

void PhpExtractor::walk_tree(TSNode node, const std::string &source,
                             const std::string &file_path, int64_t parent_id,
                             ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_php_node(type_name);
  bool is_interesting =
  ...
  if ((kind == NodeKind::Function || kind == NodeKind::Method) &&
    ...
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "function_call_expression") == 0) {
      ...
      if (strcmp(t, "member_call_expression") == 0) {
...
ExtractionResult PhpExtractor::extract(const std::string &file_path,
                                       const std::string &source) {
  ExtractionResult result;
