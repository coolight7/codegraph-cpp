 ...
 *
 * 本文件是 codegraph
 * 的核心模块，负责将源代码解析为结构化的节点（Node）和边（Edge）。
 *
 * 整体架构：
 *   源代码 → tree-sitter 解析 → AST 遍历 → 提取节点（函数/类/变量）+
 * 边（调用关系）
 *
 * 提取流程：
 ...
 *   4. 调用关系先存为 UnresolvedRef（未解析引用），后续由 main.cpp 统一解析为
 * Edge
 *
 * 设计要点：
 ...
 *
 * 为什么需要过滤：
 *   tree-sitter 没有类型信息，无法区分 `sort()` 是 std::sort 还是用户定义的
 * sort。 如果不过滤，调用图中会充满 std::sort、printf、memcpy 等噪声节点。
 *
 * 过滤策略：
 *   1. 模板实例化名以 '>' 结尾（如 vector<int>），直接过滤
 *   2. 维护一个静态集合，包含常见的 C++ 标准库、POSIX 系统调用、protobuf
 * 内部符号
 *
 * 为什么不完美：
 ...
 */
std::string CppExtractor::get_node_text(TSNode node,
 ...
 */
std::string CppExtractor::extract_signature(TSNode node,
                                            const std::string &source) {
  uint32_t start = ts_node_start_byte(node);
  uint32_t end = ts_node_end_byte(node);
  std::string full =
      source.substr(start, std::min(end, (uint32_t)source.size()) - start);
  auto pos = full.find('{');
  if (pos != std::string::npos) {
    // 去掉 '{' 前的空白字符
    while (pos > 0 && (full[pos - 1] == ' ' || full[pos - 1] == '\t' ||
                       full[pos - 1] == '\n'))
      pos--;
 ...
 */
std::string CppExtractor::extract_docstring(TSNode node,
                                            const std::string &source) {
  ...
  while (line_start > 0 && source[line_start - 1] != '\n')
    line_start--;
  if (line_start < pos && source[line_start] == '/' && line_start + 1 < pos &&
      source[line_start + 1] == '/') {
 ...
 *
 * 为什么需要这个区分：
 *   tree-sitter 把 "RpcServer server(port, registry)" 也解析为 declaration +
 * function_declarator，
...
static std::string normalize_callee_name(std::string name) {
  ...
  // 去掉成员访问前缀（obj.method → method, ptr->method → method, ns::func →
  // func）
 ...
 */
static bool should_skip_callee(TSNode function_node,
 ...
 *     - 有 function_declarator + 无 parameter_declaration →
 * 变量（构造函数初始化）
 *     - 其他 → 变量
 */
static NodeKind classify_cpp_node(const char *type_name, TSNode node) {
  ...
  if (strcmp(type_name, "declaration") == 0) {
    // 区分函数声明和变量声明：
    //   void foo(int x);                   → 函数声明（有
    //   parameter_declaration） void foo(int x) { ... }            →
    //   函数定义（有函数体） RpcServer server(port, registry);  → 变量声明（无
    //   parameter_declaration） UniqueFd fd(STDOUT_FILENO);        → 变量声明
  ...
  if (strcmp(type_name, "class_specifier") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "struct_specifier") == 0)
    return NodeKind::Struct;
  if (strcmp(type_name, "enum_specifier") == 0)
    return NodeKind::Enum;
  if (strcmp(type_name, "enumerator") == 0)
    return NodeKind::EnumMember;
  if (strcmp(type_name, "namespace_definition") == 0)
    return NodeKind::Namespace;
  if (strcmp(type_name, "type_definition") == 0)
    return NodeKind::TypeAlias;
  if (strcmp(type_name, "field_declaration") == 0)
    return NodeKind::Field;
  if (strcmp(type_name, "parameter_declaration") == 0)
    return NodeKind::Parameter;
  if (strcmp(type_name, "preproc_include") == 0)
    return NodeKind::Import;
  if (strcmp(type_name, "using_declaration") == 0)
    return NodeKind::TypeAlias;
  return NodeKind::Variable;
}
 ...
 */
void CppExtractor::walk_tree(TSNode node, const std::string &source,
 ...
 */
void CppExtractor::walk_tree_scoped(TSNode node, const std::string &source,
                                    const std::string &file_path,
                                    int64_t parent_id, const std::string &scope,
                                    ExtractionResult &result) {
    ...
    if (ts_node_is_null(name_node)) {
      ...
      for (uint32_t ni = 0; ni < named_count; ni++) {
        ...
        if (strcmp(child_type, "identifier") == 0 ||
            strcmp(child_type, "namespace_identifier") == 0) {
          name_node = child;
          break;
        }
      }
  ...
  if (is_interesting && !node_name.empty()) {
    if (kind == NodeKind::Namespace || kind == NodeKind::Class ||
  ...
  if ((kind == NodeKind::Function || kind == NodeKind::Method) &&
    ...
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "call_expression") == 0) {
        // call_expression 的结构：
        //   call_expression
        //     ├── function  → 被调用的函数（可能是 identifier、field_expression
        //     等） └── arguments → 参数列表
 ...
 */
ExtractionResult CppExtractor::extract(const std::string &file_path,
                                       const std::string &source) {
  ExtractionResult result;
...
JsExtractor::JsExtractor() : lang_(tree_sitter_javascript()) {}
JsExtractor::~JsExtractor() = default;

std::string JsExtractor::get_node_text(TSNode node, const std::string &source) {
  uint32_t start = ts_node_start_byte(node);
  uint32_t end = ts_node_end_byte(node);
  if (start >= source.size())
    return "";
...
static NodeKind classify_js_node(const char *type_name) {
  if (strcmp(type_name, "function_declaration") == 0)
    return NodeKind::Function;
  if (strcmp(type_name, "generator_function_declaration") == 0)
    return NodeKind::Function;
  ...
  if (strcmp(type_name, "import_statement") == 0)
    return NodeKind::Import;
  if (strcmp(type_name, "export_statement") == 0)
    return NodeKind::Import;
  if (strcmp(type_name, "lexical_declaration") == 0)
    return NodeKind::Variable;
...
void JsExtractor::walk_tree(TSNode node, const std::string &source,
                            const std::string &file_path, int64_t parent_id,
                            const std::string &scope,
                            ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_js_node(type_name);
  bool is_interesting =
      (kind == NodeKind::Function || kind == NodeKind::Class ||
       kind == NodeKind::Method || kind == NodeKind::Import);
  ...
  // 类定义创建新的作用域
  std::string child_scope = scope;
  if (is_interesting && !node_name.empty() && kind == NodeKind::Class) {
    child_scope = scope.empty() ? node_name : scope + "." + node_name;
  }
  ...
  for (uint32_t i = 0; i < child_count; i++) {
    TSNode child = ts_node_child(node, i);
    walk_tree(child, source, file_path, my_id, child_scope, result);
  }
  ...
  if ((kind == NodeKind::Function || kind == NodeKind::Method) &&
    ...
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      const char *t = ts_node_type(n);
      if (strcmp(t, "call_expression") == 0 ||
          strcmp(t, "new_expression") == 0) {
        TSNode fn = ts_node_child_by_field_name(n, "function", 8);
        if (ts_node_is_null(fn)) {
          fn = ts_node_child_by_field_name(n, "constructor", 11);
        }
        if (!ts_node_is_null(fn)) {
          std::string callee = get_node_text(fn, source);
          auto pos = callee.rfind('.');
...
ExtractionResult JsExtractor::extract(const std::string &file_path,
                                      const std::string &source) {
  ExtractionResult result;
...
DartExtractor::DartExtractor() : lang_(tree_sitter_dart()) {}
DartExtractor::~DartExtractor() = default;

std::string DartExtractor::get_node_text(TSNode node,
                                         const std::string &source) {
  uint32_t start = ts_node_start_byte(node);
  uint32_t end = ts_node_end_byte(node);
  if (start >= source.size())
...
static NodeKind classify_dart_node(const char *type_name) {
  if (strcmp(type_name, "function_signature") == 0)
    return NodeKind::Function;
  if (strcmp(type_name, "method_signature") == 0)
    return NodeKind::Method;
  if (strcmp(type_name, "class_definition") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "mixin_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "extension_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "enum_declaration") == 0)
    return NodeKind::Enum;
  if (strcmp(type_name, "import_specification") == 0)
    return NodeKind::Import;
  if (strcmp(type_name, "export_specification") == 0)
    return NodeKind::Import;
  return NodeKind::Variable;
}

void DartExtractor::walk_tree(TSNode node, const std::string &source,
                              const std::string &file_path, int64_t parent_id,
                              const std::string &scope,
                              ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_dart_node(type_name);
  bool is_interesting = (kind == NodeKind::Function ||
                         kind == NodeKind::Class || kind == NodeKind::Method ||
                         kind == NodeKind::Enum || kind == NodeKind::Import);
  ...
  std::string child_scope = scope;
  if (is_interesting && !node_name.empty() &&
      (kind == NodeKind::Class || kind == NodeKind::Enum)) {
    child_scope = scope.empty() ? node_name : scope + "." + node_name;
  }
  ...
  if ((kind == NodeKind::Function || kind == NodeKind::Method) &&
    ...
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      const char *t = ts_node_type(n);
      if (strcmp(t, "function_expression_invocation") == 0 ||
          strcmp(t, "selector") == 0) {
        TSNode fn = ts_node_child_by_field_name(n, "function", 8);
        if (ts_node_is_null(fn)) {
...
ExtractionResult DartExtractor::extract(const std::string &file_path,
                                        const std::string &source) {
  ExtractionResult result;
...
TsExtractor::TsExtractor() : lang_(tree_sitter_typescript()) {}
TsExtractor::~TsExtractor() = default;

TsxExtractor::TsxExtractor() : lang_(tree_sitter_tsx()) {}
TsxExtractor::~TsxExtractor() = default;

static std::string ts_get_node_text(TSNode node, const std::string &source) {
  uint32_t start = ts_node_start_byte(node);
  uint32_t end = ts_node_end_byte(node);
  if (start >= source.size())
    return "";
...
static NodeKind classify_ts_node(const char *type_name) {
  if (strcmp(type_name, "function_declaration") == 0)
    return NodeKind::Function;
  if (strcmp(type_name, "generator_function_declaration") == 0)
    return NodeKind::Function;
  ...
  if (strcmp(type_name, "class_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "abstract_class_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "method_definition") == 0)
    return NodeKind::Method;
  if (strcmp(type_name, "interface_declaration") == 0)
    return NodeKind::Class;
  if (strcmp(type_name, "enum_declaration") == 0)
    return NodeKind::Enum;
  if (strcmp(type_name, "type_alias_declaration") == 0)
    return NodeKind::TypeAlias;
  if (strcmp(type_name, "import_statement") == 0)
    return NodeKind::Import;
  if (strcmp(type_name, "export_statement") == 0)
...
static void ts_walk_tree_impl(TSNode node, const std::string &source,
                              const std::string &file_path, int64_t parent_id,
                              const std::string &scope,
                              const std::string &language,
                              ExtractionResult &result) {
  const char *type_name = ts_node_type(node);
  NodeKind kind = classify_ts_node(type_name);
  bool is_interesting =
      (kind == NodeKind::Function || kind == NodeKind::Class ||
       kind == NodeKind::Method || kind == NodeKind::Enum ||
       kind == NodeKind::TypeAlias || kind == NodeKind::Import);
  ...
  if (is_interesting) {
    ...
    TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
    if (!ts_node_is_null(name_node)) {
      n.name = ts_get_node_text(name_node, source);
    } else {
      n.name = ts_get_node_text(node, source);
      if (n.name.size() > 80)
        n.name = n.name.substr(0, 77) + "...";
    }
    node_name = n.name;
  ...
  for (uint32_t i = 0; i < child_count; i++) {
    TSNode child = ts_node_child(node, i);
    ts_walk_tree_impl(child, source, file_path, my_id, child_scope, language,
                      result);
  }
  ...
  if ((kind == NodeKind::Function || kind == NodeKind::Method) &&
    ...
    std::function<void(TSNode)> find_calls = [&](TSNode n) {
      ...
      if (strcmp(t, "call_expression") == 0 ||
        ...
        if (!ts_node_is_null(fn)) {
          std::string callee = ts_get_node_text(fn, source);
          auto pos = callee.rfind('.');
          if (pos != std::string::npos)
            callee = callee.substr(pos + 1);
...
void TsExtractor::walk_tree(TSNode node, const std::string &source,
                            const std::string &file_path, int64_t parent_id,
                            const std::string &scope,
                            ExtractionResult &result) {
  ts_walk_tree_impl(node, source, file_path, parent_id, scope, "typescript",
                    result);
}

std::string TsExtractor::get_node_text(TSNode node, const std::string &source) {
  return ts_get_node_text(node, source);
}

ExtractionResult TsExtractor::extract(const std::string &file_path,
                                      const std::string &source) {
  ExtractionResult result;
...
void TsxExtractor::walk_tree(TSNode node, const std::string &source,
                             const std::string &file_path, int64_t parent_id,
                             const std::string &scope,
                             ExtractionResult &result) {
  ts_walk_tree_impl(node, source, file_path, parent_id, scope, "tsx", result);
}

std::string TsxExtractor::get_node_text(TSNode node,
                                        const std::string &source) {
  return ts_get_node_text(node, source);
}

ExtractionResult TsxExtractor::extract(const std::string &file_path,
                                       const std::string &source) {
  ExtractionResult result;
