...
static Database open_db() {
  ...
  if (path.empty()) {
    std::cerr << "Error: No .codegraph/index found. Run 'codegraph init' first."
              << std::endl;
    exit(1);
  }
  Database db(path);
 ...
 */
static FileRecord make_file_record(const std::string &file_path,
 ...
 */
static bool is_changed(Database &db, const fs::directory_entry &entry,
 ...
 *   3. 调用提取器的 extract() 方法
 */
static bool extract_file(const std::string &file_path, const std::string &lang,
                         PendingIndexedFile &out) {
 ...
 *   整个过程在一个事务中，任何失败都会回滚。
 */
static int index_extracted_files(Database &db,
                                 std::vector<PendingIndexedFile> &files) {
  if (files.empty())
    return 0;
  ...
  try {
    ...
    for (auto &file : files) {
      ...
      for (size_t i = 0; i < file.result.nodes.size(); ++i) {
        ...
        int64_t real_id =
            (offset + 1 + i < all_ids.size()) ? all_ids[offset + 1 + i] : -1;
        if (real_id < 0) {
          db.rollback();
          return 0;
        }
    ...
    for (auto &file : files) {
      for (auto &ref : file.result.unresolved) {
        ...
        if (it != file.id_map.end())
          source_real = it->second;
        if (source_real <= 0)
          continue;
        ...
        if (src_idx >= 0 &&
            src_idx < static_cast<int>(file.result.nodes.size())) {
        ...
        if (cache_it == name_cache.end()) {
          ...
          cache_it =
              name_cache.emplace(ref.ref_name, std::move(candidates)).first;
        }
        ...
        if (!targets.empty()) {
          ...
          std::string edge_key =
              std::to_string(source_real) + ":" + std::to_string(best_id) +
    ...
    contains_edges.insert(contains_edges.end(), new_edges.begin(),
                          new_edges.end());
...
static void resolve_unresolved_refs(Database &db) {
  ...
  std::cout << "Resolving " << refs.size() << " unresolved refs..."
            << std::endl;
  ...
  std::cout << "Resolved " << new_edges.size() << " refs, "
            << (refs.size() - new_edges.size()) << " still unresolved"
            << std::endl;
 ...
 *   然后把 B 也加入重新索引列表。
 */
static void index_directory(Database &db, const std::string &path,
  ...
  auto schedule_file = [&](const std::string &file_path,
                           const std::string &lang) {
  ...
  if (!any_changed) {
    std::cout << "Indexed 0 files, 0 nodes, " << db.count_edges() << " edges"
              << std::endl;
    return;
  }
  ...
  std::cout << "Indexed " << extracted.size() << " files, " << node_count
            << " nodes, " << db.count_edges() << " edges" << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  if (cmd == "init") {
    ...
    db.init_schema();
    std::cout << "Initialized CodeGraph in " << fs::current_path().string()
              << std::endl;
    ...
    if (raw_tokens > 0) {
      std::cerr << "[codegraph] " << resp_tokens << " tokens"
                ...
                << seen_files.size() << " source files)" << std::endl;
    }
    ...
    int resp_tokens = static_cast<int>(output.size()) / 4;
    std::cerr << "[codegraph] " << resp_tokens << " tokens returned"
              << std::endl;
    ...
    fs::path script_path =
        bin_path.parent_path() / ".." / "scripts" / "embed.py";
    if (!fs::exists(script_path)) {
      script_path = fs::path(CMAKE_SOURCE_DIR) / "scripts" / "embed.py";
    }
    std::string db_path = get_db_path();
    if (db_path.empty()) {
      std::cerr << "No .codegraph/index found. Run 'codegraph init' first."
                << std::endl;
      return 1;
    }
    ...
    if (dead_nodes.empty()) {
      ...
      std::cerr << "[codegraph] Found " << dead_nodes.size()
                << " unreferenced symbols (" << resp_tokens << " tokens)"
                << std::endl;
    }
    ...
    if (cycles.empty()) {
      ...
      for (const auto &scc : cycles) {
        ...
        for (int64_t id : scc) {
          ...
          if (node.has_value()) {
            members.push_back({{"name", node->name},
                               {"file", node->file_path},
                               {"line", node->line}});
          }
        }
        output.push_back(
            {{"size", scc.size()}, {"members", std::move(members)}});
      }
      std::string result = output.dump(2);
      std::cout << result << std::endl;
    ...
    // ── path: 调用链路径查找 ──
    if (argc < 4) {
      std::cerr << "Usage: codegraph path <from_symbol> <to_symbol>"
                << std::endl;
      return 1;
    }
    Database db = open_db();
    ...
    output["to"] = {{"name", to_nodes[0].name},
    ...
    for (const auto &n : path_nodes) {
      output["path"].push_back(
          {{"name", n.name}, {"file", n.file_path}, {"line", n.line}});
    }
    std::string result = output.dump(2);
    std::cout << result << std::endl;
    ...
    std::cerr << "[codegraph] " << metrics.total_nodes << " nodes, "
              << metrics.total_edges << " edges, " << metrics.circular_deps
              << " cycles" << std::endl;
    ...
    for (const auto &in : impact) {
      ...
      output["impact"].push_back({{"name", in.node.name},
    ...
    std::string result = output.dump(2);
    std::cout << result << std::endl;
    std::cerr << "[codegraph] Impact: " << impact.size() << " symbols affected"
              << std::endl;
    ...
    if (!dot_format) {
      std::cerr
          << "Usage: codegraph export --dot [--symbol <name>] [--depth <n>]"
          << std::endl;
      return 1;
    }
    ...
    if (!symbol.empty()) {
      ...
      for (auto &e : callers.edges)
        add_edge(e);
      for (auto &e : callees.edges)
        add_edge(e);
    ...
    for (auto &e : edges) {
      ...
      if (src != node_labels.end() && dst != node_labels.end()) {
        ...
        std::cout << " [label=\"" << edge_kind_str(e.kind) << "\"];"
                  << std::endl;
      }
    }
